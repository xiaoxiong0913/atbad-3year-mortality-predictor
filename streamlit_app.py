...
if model and submit_btn:
    ...
    with col1:
        ...
        fig = go.Figure(go.Indicator(
            ...
            domain={'x': [0, 1], 'y': [0, 1]},
            title={
                'text': f"3-Year MACE Probability<br><span style='font-size:0.8em;color:gray'>Threshold: {THRESHOLD * 100}%</span>"},
    ...
    with col2:
        ...
        if inputs['HGB(g/L)'] < hgb_thresh:
            alerts.append(
                f"🟠 <b>Hemoglobin ({inputs['HGB(g/L)']} g/L):</b> Below normal for {gender} (<{hgb_thresh}). Evaluate for anemia.")
    ...
    with st.spinner("Generating explanation..."):
        try:
            ...
            st.caption(
                "Interactive Force Plot: Bars pushing to the right (Red) increase risk, while bars pushing to the left (Blue) decrease risk.")
